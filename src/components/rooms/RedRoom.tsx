...
export function RedRoom({ onBack }: RedRoomProps) {
  const [events, setEvents] = useState<Event[]>([
    { id: "1", title: "Maintenance Check", time: "08:00 - 10:00", attendees: 3, date: "Nov 10, 2025", room: "mechanical" },
    { id: "2", title: "Storage Inventory", time: "14:00 - 16:00", attendees: 2, date: "Nov 12, 2025", room: "storage" }
  ]);
  ...
  const rooms: Room[] = [
    { id: "storage", name: "Storage Room", x: "10%", y: "10%", width: "35%", height: "35%", color: "#f5f5f5" },
    { id: "parking", name: "Parking Area", x: "50%", y: "10%", width: "40%", height: "50%", color: "#e5e5e5" },
    { id: "mechanical", name: "Mechanical Room", x: "10%", y: "50%", width: "30%", height: "40%", color: "#d4d4d4" },
    { id: "electrical", name: "Electrical Room", x: "45%", y: "65%", width: "22%", height: "25%", color: "#c4c4c4" },
    { id: "archive", name: "Archive", x: "72%", y: "65%", width: "18%", height: "25%", color: "#b5b5b5" }
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-6">
      {/* Draggable Action Circles with Context Provider */}
      <DraggableActionCircles rooms={rooms}>
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="px-6 py-2 bg-gray-800 text-white rounded-full">Basement (Level 1)</div>
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-2 bg-white rounded-2xl shadow-xl p-8">
              ...
              <div className="aspect-[4/3] bg-gray-100 rounded-xl p-4 relative border-4 border-gray-800">
                ...
                <div className="absolute left-[45%] top-0 w-[10%] h-full bg-gray-300 opacity-50" />
              </div>
              <div className="mt-4 text-sm text-gray-600"><p>Total Floor Area: 520 m² | Floor: Basement</p></div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
              {selectedRoom ? (
                <div className="bg-white rounded-2xl shadow-xl p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <Button onClick={() => setIsAddEventOpen(true)} size="sm" className="bg-gray-800 hover:bg-gray-900">
                      ...
                      <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        {getRoomEvents(selectedRoom.id).map((event) => (
                          <div key={event.id} className="p-3 bg-gray-50 rounded-lg border-l-3 border-gray-800 relative group">
                            <button onClick={() => handleDeleteEvent(event.id)} className="absolute top-2 right-2 opacity-0 group-hover:opacity-100">
              ...
              {/* Room Statistics Panel - Below All Events */}
              <RoomStatisticsPanel />
            </motion.div>
          </div>
        </div>
      </DraggableActionCircles>

      <Dialog open={isAddEventOpen} onOpenChange={setIsAddEventOpen}>
        <DialogContent className="bg-white">
          <DialogHeader>
            <DialogTitle>Add Event to {selectedRoom?.name}</DialogTitle>
            <DialogDescription>Schedule a new event in this room</DialogDescription>
          </DialogHeader>
